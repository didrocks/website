...
Our grub bootloader (available via [Escape] or [Shift] press on boot) will enable you to revert the system (and optionally user) states on demand! An history entry will allow you to boot on an older state of your system.

This goes further than OpenZFS rollbacks as we made revert a non destructive action: current and intermediate states aren’t destroyed, and you can imagine even reverting the revert (or doing system bisection!). Also, our revert don’t apply base filesystem dataset properties, but store exactly some key dataset properties as to when the state saving was taken, to ensure better fidelity when reverting. For instance, changing a filesystem mountpoint won’t apply to the snapshot inheriting from it, and we will remount it at the correct place. Similarly, we will reboot with the exact same kernel you booted the state that you saved with, even if this wasn’t the latest available versions.
...
Similarly, we save and commit every time you are able to boot successfully your system (we will define successful boot in the next blog post about states). This can trigger a grub menu rebuild if needed (new states to add or so). It means that if a boot is failing and you revert, just rebooting should keep the latest successful state as the default grub entry!

This is just a taste of it on our path of robust and bullet-proof Ubuntu desktop. We will explain all of this in greater details in the next blog post.
...
This allows us to have to take automatic hourly user state save, but doing that only if the user is connected (to a GUI or CLI session)!
...
User snapshots are basically free when you don’t change file (you can think of them as if they were only storing the difference between current and previous states). It will allow us in the near future to offer easily restore of previous version of your user’s data. The "oh, I removed that file and don't have backups" will be something of the past soon :)

Note that this isn’t a real backup as all your data are on the same physical place or disk, so this shouldn’t replace backups. We have plan to integrate that with some backup tools in the future.


## Garbage collection on state saving

As you can infer from the above, we will have a lot of state saves. While we allow the user to manually save and remove states, as most of them will be taken automatically, it would be complicated and counter-productive on a daily bases to? Add to this that some states are dependent on other states to be purged (more on that in … you would have guess, the next blog post about state!), you can understand the complexity there.

This will have also its dedicated post, but in summary, we are trying to prune states as time passes, to ensure that you have a number of relevant state that you can revert to. The general idea is that more time pass by, the less granularity you will have to revert to to save disk space. You will have a very finer grain states to revert to for the previous day, a little bit less for the previous weeks, less for months… You get it :)
...
Similarly, we don’t want zsys to slow down or interfere in the boot. This is why we hooked it up to the upstream OpenZFS systemd generator and it will only be run when doing a revert to a previous state.

### Everything is stored on ZFS pools

Secondly, we don’t want to maintain a separate database for ZFS dataset layout and structure. This approach would have been dangerous as being out of sync with what is really on disk, also being unaware of any manual user interaction on ZFS. That way, we let ZFS familiar system administrators handling manual operations while still being compatible with Zsys. Any additional properties we need are handled via user properties on ZFS datasets directly. Basically, everything is in the ZFS pool and no additional data are needed (you can migrate your disk from one disk to another).
...
Finally, familiarity and discoverability. `zsysctl` has a lot of commands, subcommands and options. We are using advanced shell completion which will complete on **[Tab][Tab]** in both bash and zsh environments.
```sh
$ zsysctl state [Tab][Tab]
remove  save
```
We try to discard any optional arguments by limiting the number of them. However if you try to complete on `-` or ``--``, we will present any available matching options (some being global, some being local to your subcommand):
```sh
...
Similarly, an help command is available for each commands and subcommands as well as completion:
```sh
...
Also, all those commands and subcommands are backed by man pages. Those are for now the equivalent of --help but if you have the desire to enhance any of them, this is a simple, but very valuable contribution that we welcome strongly welcome as [upstream Pull Requests](https://github.com/ubuntu/zsys)!
...
If you want to implement something similar in your (golang) CLI program, we [proposed upstream our changes](https://github.com/spf13/cobra/pull/983) to the cobra upstream repository so that you can get advanced shell completion as well!

### Versatility

We didn’t want to force upon user a strict zsys layout. First, we will have different ZFS default dataset layouts between server and desktop (this will be detailed in a future blog post). We are also very aware that there are exiting and passionate ZFS system administrator or hobbyist that want to have full control over their system. This is why:
* Any system can be untagged so prevent zsys controlling it. The system will then boot and behave as any ZFS on root manual installation. Aware, of course, any additional features we are providing through Zsys will unavailable.
